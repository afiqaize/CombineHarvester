...
def generate_g_grid(pair, ggrids = "", gmode = "", propersig = False, grange = default_g_range(), initial_distance = (1., 1.), randaround = (-1., -1.), randnminmax = default_nminmax()):
    if not hasattr(generate_g_grid, "alphas"):
        ...
        generate_g_grid.nomore = 49 # n passing toys above which we declare the point is 'confidently' inside
        generate_g_grid.atleast = 9 # n total toys (divided by alphas) below which we consider it unreliable to say a point is in/outside
